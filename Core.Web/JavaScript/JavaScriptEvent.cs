    ...
    public class JavaScriptEvent
    {
        ...
        public string Render()
        {
            ...
            if (this._delegate != null)
            {
                if (this._id != default)
                {
                    @event = $"$(\"#{this._id.Value}\").on('{this._event.EventString()}',function(){{{this._delegate}(event.currentTarget);}});";
                }
                else if (this._class != default)
                {
                    @event = $"$(\".{this._class}\").on('{this._event.EventString()}',function(){{{this._delegate}(event.currentTarget);}});";
                }
            }
